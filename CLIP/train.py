...
def train(config, train_dataset, model):
    ...
    for epoch in range(int(config.num_train_epochs)):
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % config.gradient_accumulation_steps == 0:
                ...
                model.zero_grad()
                loss_values.append(global_loss / global_step)
    ...
    plt.legend()
    plt.savefig('./training-plots/training-loss-ViT.png')  
...
def main():
    ...
    model = CLIP(**model_params)
    # print('model_params[embed_dim]',model_params['embed_dim'])
    # print(model_params['vision_patch_size'])
